...
// SaveURL provides a mock function with given fields: urlSaver, alias
func (_m *URLSaver) SaveURL(urlSaver string, alias string) (int64, error) {
	ret := _m.Called(urlSaver, alias)
	...
	if rf, ok := ret.Get(0).(func(string, string) (int64, error)); ok {
		return rf(urlSaver, alias)
	}
	if rf, ok := ret.Get(0).(func(string, string) int64); ok {
		r0 = rf(urlSaver, alias)
	} else {
		r0 = ret.Get(0).(int64)
	}

	if rf, ok := ret.Get(1).(func(string, string) error); ok {
		r1 = rf(urlSaver, alias)
	} else {
		r1 = ret.Error(1)
	}
